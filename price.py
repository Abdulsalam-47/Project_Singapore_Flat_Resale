...
# Assuming you have a path to your GIF image
image_path = r'D:\Guvi Projects\Singapore Resale Flat Prices Prediction\Animated Logo.gif'

# Displaying the image with correct parameters
st.image(image_path, use_column_width=True, width=250)

st.markdown("<h1 style='text-align: center; color: purple;'>Singapore Flat Resale Price Predicton</h1>",unsafe_allow_html=True)

df=pd.read_csv(r'D:\Guvi Projects\Singapore Resale Flat Prices Prediction\final.csv')
df=df.drop(['Unnamed: 0'],axis=1) 

tab1,tab2,tab3=st.tabs(['HOME','FILL THE DETAILS','PREDICTION AND CONCLUSION'])
...
with tab3:
    ...
    if st.button('Predict and Conclude'):
            
            def load_data():
                # Load data
                data = pd.read_csv(r'D:\Guvi Projects\Singapore Resale Flat Prices Prediction\final.csv')
